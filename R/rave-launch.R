...
#' @param jupyter whether to launch \code{'Jupyter'} server
...
debug_modules <- function(module_root = rstudioapi::getActiveProject(), host = '127.0.0.1', port = 17283, jupyter = TRUE, ...){
  ...
  if(jupyter){
    ...
    rpymat::jupyter_check_launch(open_browser = FALSE, workdir = jupyter_wd, port = jupyter_port, host = '127.0.0.1', async = TRUE)
  }
